          ...
          <div className="relative max-w-4xl mx-auto w-full px-4 overflow-x-hidden">
            ...
            {APP_CONTENT.about.timeline.map((item, idx) => (
              <motion.div
                ...
                className={`relative flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-8 mb-8 sm:mb-12 w-full ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''
                  }`}
              >
                {/* Content Side */}
                <div className="flex-1 w-[calc(100%-3.5rem)] md:w-[calc(50%-1rem)] ml-10 md:ml-0 min-w-0">
                  <div className={`p-4 sm:p-6 bg-white shadow-xl rounded-lg border border-gray-100 relative w-full break-words-safe ${idx % 2 === 0 && 'md:text-right'}`}>
